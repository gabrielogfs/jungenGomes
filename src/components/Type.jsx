    ...
    <div>
      <section className="bg-gray-100 pt-4 px-4">
        <div className="max-w-5xl mx-auto">
          <h1 className="text-3xl font-bold italic text-slate-900 mb-6 text-center">
            {typeId}
          </h1>
          {loading && <h1>Carregando...</h1>}
          {!loading && (
            <ul className="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-4 items-center justify-center mx-auto">
              {productType.map(({ id, name, img, stock, price, weapon }) => (
                <li
                  className="w-56 m-4 border-2 border-red-700 rounded-lg shadow-lg hover:shadow-2xl hover:scale-105 transition-transform duration-200 overflow-hidden"
                  key={id}
                >
                  <Link to={`/product/${id}`}>
                    <img
                  ...
                  <div className="p-4">
                    ...
                    <div className="font-light italic text-slate-900 text-center mb-2">
                      {weapon}
                    </div>
                  </div>
